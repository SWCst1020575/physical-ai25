...
class Environment(gym.Env):
    ...
    def __init__(self,
        ...
        # IK performance profiling (pybullet_ik by default)
        self._ik_solver_name = 'your_ik'
        self._ik_calls = 0
        self._ik_time_total = 0.0
        self._ik_time_max = 0.0
...
# Feel free to make changes if desired; otherwise, there's no need for modifications.
    ...
    def solve_ik(self, pose):
        ...
        your_joints = your_ik(self.ur5, new_pose,
                              max_iters=100,  base_pos=[0, 0, 0])
        
        # your_joints = pybullet_ik(self.ur5, new_pose,
        #                               max_iters=100,  base_pos=[0, 0, 0])
        dt = time.perf_counter() - t0
        self._ik_calls += 1
        self._ik_time_total += dt
        if dt > self._ik_time_max:
            self._ik_time_max = dt
    ...
    # IK profiling helpers
    def reset_ik_stats(self):
        self._ik_calls = 0
        self._ik_time_total = 0.0
        self._ik_time_max = 0.0

    def get_ik_stats(self):
        avg = (self._ik_time_total / self._ik_calls) if self._ik_calls > 0 else 0.0
        return {
            'solver': self._ik_solver_name,
            'calls': int(self._ik_calls),
            'total_time_s': float(self._ik_time_total),
            'avg_time_s': float(avg),
            'max_time_s': float(self._ik_time_max),
        }
